...
//Set variable to equal a new array
let allCaps = [];
//Use .map on the array runners and create an arrow function with runner as the place holder of each iterated upon object within the array
runners.map((runner) => {
 //Set the new array that was declared above to have the all capitalized first name of each first name object within the runners array (runner here is the place holder of each array object) and place them in the last position inside of the newly created array allCaps
 allCaps.push(runner.first_name.toUpperCase());
})
//Print out the allCaps array to the console
console.log(allCaps);
...
//Create the new variable that is assigned to an empty array
let largeShirts = [];
//Use filter on the array runners and use the placeholder runner
runners.filter((runner) => {
 //Create an if statement that checks to see if the specified array value has a shirt size of "L" 
 if (runner.shirt_size === 'L' ){
  //If it does, then we are to take the first name, the last name, and the email of the user from that array object and push those values using string interpolation into the new array created above - largeShirts
  largeShirts.push(` ${runner.first_name} ${runner.last_name} ${runner.email}`)
  //Returns the value of the largegShirts array that now contains the new value
  return largeShirts
 } 
})
...
totalCost = (acc, cur) => acc + cur;

runners.reduce((ticket) => {
 if (ticket.donation = ""){
  //Leave empty
 }else {
